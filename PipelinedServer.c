...
char *generate_status_line(client_request *req_object, response_status *res_status) {
    ...
    strcpy(res_status->version, "HTTP/1.0");
    ...
    // Check for valid HTTP protocol (Only HTTP/1.0 or 1.1)
        ...
        status = "HTTP/1.0 505 HTTP Version Not Supported\n";
...
char *generate_response(client_request *req_object) {
    ...
    if(strcmp(res_status.res_code, "304") == 0) {
        ...
        strcpy(response, "HTTP/1.0 304 Not Modified\r\n");
...
int parse_request(char *req_line, client_request *req_object, char *root_path) {
    ...
    const char delim[2] = " ";
    char path_buf[512];
    char *token, *req_method, *req_path = "\0", *req_version = "HTTP/1.0";
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        while (bytes_read > 0) {
            ...
            char *cli_request = multi_tok(client_msg, "\r\n\r\n");

            while (cli_request != NULL) {
                if (strcmp(cli_request, "") != 0) {
                    line = strtok(cli_request, "\r\n");
                    while (line != NULL) {
                        char to_send[128];
                        strcpy(to_send, line);
                        if (i == 0) {
                            parse_request(to_send, &req_object, http_root_path);
                        } else {
                            parse_headers(to_send, &req_object);
                        }
                        i++;
                        line = strtok(NULL, "\r\n");
                    }
                    ...
                    if (req_object.headers[0].name != NULL) {
                        free(req_object.headers);
                    }
                    free(server_response);
                }
                i = 0;
                cli_request = multi_tok(NULL, "\r\n\r\n");
            }
            bytes_read = read(client_fd, client_msg, BUF_SIZE - 1);
        }
    }
